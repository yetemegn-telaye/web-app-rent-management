...
const Sidebar = () => {
  ...
  return (
    <aside className={`bg-white shadow-lg h-100vh mt-1 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
      <div className="p-4">
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="text-gray-700 focus:outline-none"
        >
          <FontAwesomeIcon icon={isCollapsed ? faChevronCircleRight : faChevronCircleLeft} />
        </button>
        ...
        <nav className="space-y-8 px-4 text-primary-dark font-medium mt-6">
          ...
          {userType === 'manager' && (
            <div className="flex flex-col gap-1 mb-6">
              <h2 className={`text-xs text-accent uppercase tracking-wider mb-2 ${isCollapsed ? 'hidden' : ''}`}>Menu</h2>
              <Link to="/dashboard" className="flex items-center p-2 hover:bg-gray-200 rounded">
          ...
          <div className="flex flex-col gap-1">
            <h2 className={`text-xs text-accent uppercase tracking-wider mb-2 ${isCollapsed ? 'hidden' : ''}`}>Listings</h2>
          ...
          {userType === 'manager' && (
            <div className="flex flex-col gap-1">
              <h2 className={`text-xs text-accent uppercase tracking-wider mb-2 ${isCollapsed ? 'hidden' : ''}`}>Tenants</h2>
          ...
          {userType === 'manager' && (
            <div className="flex flex-col gap-1">
              <h2 className={`text-xs text-accent uppercase tracking-wider mb-2 ${isCollapsed ? 'hidden' : ''}`}>Administration</h2>
