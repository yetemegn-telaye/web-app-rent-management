...
const AddListing: React.FC = () => {
  ...
  const handleNextStep = (nextOption: string) => {
    ...
    const nextStepIndex = buttonOptions.findIndex(
      (option) => option.label === nextOption
    );
  ...
  const renderContent = () => {
    switch (selectedOption) {
      case "Basic Detail":
        return (
          <BasicDetailForm
            setSelectedOption={() => handleNextStep("Special Features")}
          />
        );
  ...
  return (
    <LandlordLayout>
      <div className="px-2 md:px-4">
        ...
        {/* Sticky Stepper */}
        <div className="sticky top-20 z-20 bg-primary/80 backdrop-blur-md py-2">
          <FormStepper
        ...
        {/* Form Content */}
        <div className="flex flex-col mt-6">{renderContent()}</div>
      </div>
    </LandlordLayout>
  );
};
